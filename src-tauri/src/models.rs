...
/// el progreso por `on_progress(descargado, total)`. Reusa el helper de descarga
/// compartido (`download`), que baja a un `.part` y lo renombra al terminar.
pub fn download(dir: &Path, id: &str, on_progress: impl Fn(u64, u64)) -> Result<ModelInfo, String> {
    ...
    crate::download::download_file(&url, &destination, on_progress)?;
    Ok(info(dir, level))
}
